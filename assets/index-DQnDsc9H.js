(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&a(d)}).observe(document,{childList:!0,subtree:!0});function o(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=o(n);fetch(n.href,r)}})();let B=!1,y="",f="menu";const u="https://admin-site-ze7d.onrender.com",S="df860f2526805007d06289b53d901e26",k="https://api.the-odds-api.com/v4";function P(e){let t=e.value.replace(/[^\d]/g,"");if(!t){e.value="";return}t=parseInt(t,10).toString(),e.value=t.replace(/\B(?=(\d{3})+(?!\d))/g,","),e.value!==""&&(e.value="$"+e.value)}function x(e){return e?e.replace(/[^\d.\-]/g,""):""}function N(e){let t=e.value.replace(/[^\d]/g,"");t.length>10&&(t=t.slice(0,10));let o=t;t.length>6?o=t.slice(0,3)+"-"+t.slice(3,6)+"-"+t.slice(6):t.length>3&&(o=t.slice(0,3)+"-"+t.slice(3)),e.value=o}function M(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("loginForm").onsubmit=async e=>{e.preventDefault();const t=document.getElementById("loginUsername").value,o=document.getElementById("loginPassword").value;document.getElementById("loginLoading").style.display="block",document.getElementById("loginError").innerText="";try{const a=await fetch(`${u}/api/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:t,password:o})});if(a.ok)B=!0,y=t,f="menu",h();else{const n=await a.json();document.getElementById("loginError").innerText=n.message||"Login failed"}}catch{document.getElementById("loginError").innerText="Connection failed. Please try again."}finally{document.getElementById("loginLoading").style.display="none"}}}function I(e=8){const t="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";let o="";for(let a=0;a<e;a++)o+=t.charAt(Math.floor(Math.random()*t.length));return o}function L(e){return e.split(".").pop()}function U(){return"PB-"+I(10)}function j(e){const t=L(e.name);return`GeneralsGamblingAdmin_${I(12)}.${t}`}async function v(e){const t="tAhG8ZNH6lBSEf0xnJT2aOuDP7Jiu9u7",o=j(e),a=new FormData;a.append("file",e,o),a.append("metadata",JSON.stringify({name:o}));const n=await fetch("https://fmapi.net/api/v2/image",{method:"POST",headers:{Authorization:t},body:a}),r=await n.json();if(n.ok&&r.data&&r.data.url)return{url:r.data.url,filename:o};throw new Error(r?.message||"Image upload failed.")}async function q(e){const t="tAhG8ZNH6lBSEf0xnJT2aOuDP7Jiu9u7",o=new FormData,a=L(e.name),n=`GeneralsGamblingAdmin_${I(12)}.${a}`;o.append("file",e,n),o.append("metadata",JSON.stringify({name:n}));const r=await fetch("https://fmapi.net/api/v2/image",{method:"POST",headers:{Authorization:t},body:o}),d=await r.json();if(r.ok&&d.data&&d.data.url)return d.data.url;throw new Error(d?.message||"Image upload failed.")}function h(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("addFundsBtn").onclick=()=>{f="addFunds",_()},document.getElementById("sportsBetsBtn").onclick=()=>{f="sportsBets",O()},document.getElementById("raffleTicketsBtn").onclick=()=>{f="raffleTickets",z()},document.getElementById("propBetsBtn").onclick=()=>{f="propBets",W()},document.getElementById("logoutBtn").onclick=()=>{B=!1,y="",f="menu",M()},document.getElementById("swapSheetBtn").onclick=J}async function J(){const e=document.getElementById("sheetSelect").value,t=document.getElementById("swapStatus");if(!e){t.innerHTML='<span style="color: orange;">Please select a sheet first.</span>';return}t.innerHTML='<span style="color: blue;">Swapping sheets...</span>';try{const o=await fetch(`${u}/api/swap-sheet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({targetSheet:e,adminUsername:y})}),a=await o.json();o.ok?a.success?t.innerHTML=`<span style="color: green;">✓ Successfully swapped to ${e} sheet!</span>`:(await E(e),t.innerHTML=`<span style="color: green;">✓ Successfully swapped to ${e} sheet (fallback method)!</span>`):(await E(e),t.innerHTML=`<span style="color: green;">✓ Successfully swapped to ${e} sheet (fallback method)!</span>`)}catch(o){console.error("Sheet swap error:",o);try{await E(e),t.innerHTML=`<span style="color: green;">✓ Successfully swapped to ${e} sheet (fallback method)!</span>`}catch{t.innerHTML='<span style="color: red;">✗ Failed to swap sheets. Please try again.</span>'}}setTimeout(()=>{t.innerHTML=""},3e3)}async function E(e){const t=await fetch(`${u}/api/swap-sheet-fallback`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({targetSheet:e,adminUsername:y})});if(!t.ok)throw new Error("Fallback sheet swap failed");return await t.json()}function O(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("backToMenuBtn").onclick=()=>{f="menu",h()},X(),document.getElementById("sportSelect").addEventListener("change",function(){const e=this.value;e?ee(e):document.getElementById("oddsContainer").innerHTML="<p>Select a sport to view available odds</p>"})}function _(){document.querySelector("#app").innerHTML=`
    <div style="display: flex; flex-direction: column; align-items: center; gap: 1rem;">
      <button id="backToMenuBtn" style="padding: 0.5rem 1rem; background: #666; color: #fff; border: none; border-radius: 5px; font-size: 0.9rem; cursor: pointer; width: auto;">← Back to Menu</button>
      <div class="form-container">
  ...
  `,document.getElementById("exampleBtn").onclick=()=>{document.getElementById("exampleModal").style.display="flex"},document.getElementById("closeExampleModal").onclick=()=>{document.getElementById("exampleModal").style.display="none"},document.getElementById("backToMenuBtn").onclick=()=>{f="menu",h()};const e=document.getElementById("proofImage");document.getElementById("playerForm").addEventListener("paste",o=>{if(o.clipboardData&&o.clipboardData.files&&o.clipboardData.files.length>0){const a=o.clipboardData.files[0];if(a.type.startsWith("image/")){const n=new DataTransfer;n.items.add(a),e.files=n.files}}}),document.getElementById("amountToAdd").addEventListener("input",function(){P(this)}),document.getElementById("playerForm").onsubmit=async o=>{o.preventDefault();const a=document.getElementById("playerUsername").value,n=x(document.getElementById("amountToAdd").value),r=document.getElementById("proofImage").files[0];if(!r){document.getElementById("formError").innerText="Image required.";return}let d="",l="";try{const{url:m,filename:b}=await v(r);d=m,l=b}catch(m){document.getElementById("formError").innerText=m.message;return}const i={playerUsername:a,amountToAdd:n,proofImageUrl:d,proofImageFilename:l,adminUsername:y},c=await fetch(`${u}/api/submit`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)}),s=await c.json();c.ok?(s.newAccount?(document.getElementById("formError").innerHTML=`Error, account name, (<b>${a}</b>) not found. <br /> Please contact the general to fix the error. <br /> Their money is still tracked to (<b>${a}</b>) for now.`,document.getElementById("formError").style.color="red",document.getElementById("formSuccess").innerText=""):(document.getElementById("formSuccess").innerHTML=`Submitted successfully to <b>${a}</b>!`,document.getElementById("formError").innerText=""),document.getElementById("playerForm").reset()):(document.getElementById("formError").innerText=s.message||"Submission failed.",document.getElementById("formSuccess").innerText="")}}function z(){document.querySelector("#app").innerHTML=`
    <div style="display: flex; flex-direction: column; align-items: center; gap: 1rem;">
      <button id="backToMenuBtn" style="padding: 0.5rem 1rem; background: #666; color: #fff; border: none; border-radius: 5px; font-size: 0.9rem; cursor: pointer; width: auto;">← Back to Menu</button>
      <div class="form-container">
  ...
  `,document.getElementById("backToMenuBtn").onclick=()=>{f="menu",h()},document.getElementById("propExampleBtn").onclick=()=>{document.getElementById("propExampleModal").style.display="flex"},document.getElementById("closePropExampleModal").onclick=()=>{document.getElementById("propExampleModal").style.display="none"},document.getElementById("propBetForm").onsubmit=Y,w()}async function G(){const e=await fetch(`${u}/api/players`);return e.ok?await e.json():[]}async function R(e,t){await fetch(`${u}/api/deduct-balance`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:e,amount:t})})}async function Y(e){e.preventDefault();const t=document.getElementById("propPlayers").value.trim(),o=document.getElementById("propBetName").value.trim(),a=document.getElementById("propHowToWin").value.trim(),n=parseFloat(x(document.getElementById("propAmount").value)),r=document.getElementById("propPasscode").value.trim(),d=Array.from(document.getElementById("propProofImages").files),l=t.split(",").map(g=>g.trim()).filter(Boolean);if(!l.length||!o||!a||!n||!r||!d.length){document.getElementById("propFormError").innerText="All fields are required.";return}document.getElementById("propFormError").innerText="",document.getElementById("propFormSuccess").innerText="Uploading images...";let i=[];try{for(const g of d){const{url:D,filename:H}=await v(g);i.push({url:D,filename:H})}}catch(g){document.getElementById("propFormError").innerText=g.message,document.getElementById("propFormSuccess").innerText="";return}const c=U(),s=Math.round(n*.1*100)/100,m=Math.round((n*l.length+s)*100)/100,b=new Date().toLocaleString(),A=i.map(g=>g.url),C=i.map(g=>g.filename),$=[c,o,a,n,s,r,l.join(","),b,m,"active",...A,...C];await fetch(`${u}/api/submit-prop-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({row:$,adminUsername:y})}),document.getElementById("propFormSuccess").innerText="Prop bet submitted!",document.getElementById("propFormError").innerText="",document.getElementById("propBetForm").reset(),w()}async function w(){const e=await fetch(`${u}/api/active-prop-bets`),t=e.ok?await e.json():[],o=document.getElementById("propActiveDropdownContainer");o.innerHTML=`
  ...
  `,document.getElementById("propJoinBtn").onclick=()=>K(t)}async function K(e){const t=document.getElementById("propActiveDropdown").value.trim(),o=e.find(a=>a.betName===t);if(!o){document.getElementById("propJoinError").innerText="Bet not found.";return}Z(o)}function Z(e){let t=document.getElementById("propJoinModal");t&&t.remove(),t=document.createElement("div"),t.id="propJoinModal",t.style="position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.7); display:flex; align-items:center; justify-content:center; z-index:2000;",t.innerHTML=`
  ...
  `,document.getElementById("propAddToBetForm").onsubmit=a=>V(a,e,t)}async function V(e,t,o){e.preventDefault();const a=document.getElementById("propAddPlayers").value.trim(),n=Array.from(document.getElementById("propAddProofImages").files),r=a.split(",").map(i=>i.trim()).filter(Boolean);if(!r.length||!n.length){document.getElementById("propAddToBetError").innerText="All fields required.";return}document.getElementById("propAddToBetError").innerText="",document.getElementById("propAddToBetSuccess").innerText="Uploading images...";let d=[];try{for(const i of n){const c=await q(i);d.push(c)}}catch(i){document.getElementById("propAddToBetError").innerText=i.message,document.getElementById("propAddToBetSuccess").innerText="";return}const l=await G();for(const i of r)l.some(s=>s.username===i)&&await R(i,t.amount);await fetch(`${u}/api/join-prop-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({betID:t.betID,addPlayers:r.join(","),addProofUrls:d})}),document.getElementById("propAddToBetSuccess").innerText="Added to bet!",document.getElementById("propAddToBetError").innerText="",document.getElementById("propAddToBetForm").reset(),setTimeout(()=>o.remove(),2e3),w()}async function X(){try{const e=await fetch(`${k}/sports?apiKey=${S}`);if(!e.ok)throw new Error(`HTTP error! status: ${e.status}`);const t=await e.json(),o=document.getElementById("sportSelect");o.innerHTML='<option value="">Select a sport...</option>',t.filter(n=>n.active).sort((n,r)=>n.group===r.group?n.title.localeCompare(r.title):n.group.localeCompare(r.group)).forEach(n=>{const r=document.createElement("option");r.value=n.key,r.textContent=`${n.group} - ${n.title}`,o.appendChild(r)})}catch(e){console.error("Error loading sports:",e),document.getElementById("sportSelect").innerHTML='<option value="">Error loading sports</option>'}}async function ee(e){const t=document.getElementById("oddsContainer");t.innerHTML="<p>Loading odds...</p>";try{const o=await fetch(`${k}/sports/${e}/odds?regions=us&oddsFormat=decimal&markets=h2h,spreads,totals&apiKey=${S}`);if(!o.ok)throw new Error(`HTTP error! status: ${o.status}`);const a=await o.json(),n={title:te(e),events:a};oe(n)}catch(o){console.error("Error loading odds:",o),t.innerHTML="<p>Error loading odds. Please try again.</p>"}}function te(e){return{americanfootball_nfl:"NFL",americanfootball_ncaaf:"NCAAF",basketball_nba:"NBA",basketball_wnba:"WNBA",baseball_mlb:"MLB",icehockey_nhl:"NHL",soccer_epl:"EPL",soccer_usa_mls:"MLS",mma_mixed_martial_arts:"MMA"}[e]||e}function oe(e){const t=document.getElementById("oddsContainer");if(!e.events||e.events.length===0){t.innerHTML="<p>No events available for this sport at the moment.</p>";return}let o=`<h3>${e.title} - Upcoming Games</h3>`;e.events.forEach((r,d)=>{const l=new Date(r.commence_time).toLocaleString();o+=`
  ...
  `,e.events.forEach((r,d)=>{const l=document.getElementById(`event-card-${d}`),i=document.getElementById(`event-content-${d}`);l&&i&&(i.style.display="none",l.classList.add("collapsed"),l.addEventListener("click",function(c){c.target.closest(".event-content")||(i.style.display==="none"?(i.style.display="block",l.classList.remove("collapsed")):(i.style.display="none",l.classList.add("collapsed")))}))}),document.querySelectorAll(".betting-cell").forEach(r=>{r.addEventListener("click",function(d){d.stopPropagation(),ne(this)})}),document.getElementById("closeBettingModal").addEventListener("click",F),document.getElementById("closeBettingExampleModal").addEventListener("click",re),document.getElementById("bettingExampleBtn").addEventListener("click",()=>{document.getElementById("bettingExampleModal").style.display="flex"}),document.getElementById("bettingAmount").addEventListener("input",function(){P(this)});const n=document.getElementById("bettingProofImage");document.getElementById("bettingForm").addEventListener("paste",r=>{if(r.clipboardData&&r.clipboardData.files&&r.clipboardData.files.length>0){const d=r.clipboardData.files[0];if(d.type.startsWith("image/")){const l=new DataTransfer;l.items.add(d),n.files=l.files}}}),document.getElementById("bettingForm").addEventListener("submit",ae)}function T(e){return{h2h:"Moneyline",spreads:"Spread",totals:"Total"}[e]||e}let p=null;function ne(e){const t=e.dataset.event,o=e.dataset.eventTime;let a=e.dataset.betType;const n=e.dataset.selection,r=e.dataset.odds,d=e.dataset.line;let l=a;typeof a=="string"&&a.trim().toLowerCase()==="moneyline"&&(l="Moneyline/h2h"),p={event:t,eventTime:o,betType:l,selection:n,odds:r,line:d},document.getElementById("modalEventTitle").textContent=`${t} - ${l}`;const i=document.getElementById("teamSelection");if(i.innerHTML="",a.trim().toLowerCase()==="moneyline")t.split(" @ ").forEach(s=>{const m=document.createElement("button");m.type="button",m.textContent=s,m.style.cssText="padding:0.5rem 1rem; background:#333; color:#fff; border:1px solid #444; border-radius:5px; cursor:pointer; flex:1;",m.addEventListener("click",function(){i.querySelectorAll("button").forEach(b=>{b.style.background="#333",b.style.borderColor="#444"}),this.style.background="#646cff",this.style.borderColor="#646cff",p.selectedTeam=this.textContent}),i.appendChild(m)});else{const c=document.createElement("button");c.type="button",c.textContent=n,c.style.cssText="padding:0.5rem 1rem; background:#333; color:#fff; border:1px solid #444; border-radius:5px; cursor:pointer; flex:1;",c.addEventListener("click",function(){i.querySelectorAll("button").forEach(s=>{s.style.background="#333",s.style.borderColor="#444"}),this.style.background="#646cff",this.style.borderColor="#646cff",p.selectedTeam=this.textContent}),i.appendChild(c)}}function F(){document.getElementById("bettingModal").style.display="none",p=null}function re(){document.getElementById("bettingExampleModal").style.display="none"}async function ae(e){if(e.preventDefault(),!p||!p.selectedTeam){document.getElementById("bettingError").textContent="Please select a team.";return}const t=document.getElementById("bettingUsername").value,o=x(document.getElementById("bettingAmount").value),a=document.getElementById("bettingProofImage").files[0];if(!a){document.getElementById("bettingError").textContent="Image required.";return}document.getElementById("bettingSuccess").textContent="Processing... Please wait.",document.getElementById("bettingError").textContent="";let n="",r="";try{const{url:l,filename:i}=await v(a);n=l,r=i}catch(l){document.getElementById("bettingError").textContent=l.message,document.getElementById("bettingSuccess").textContent="";return}const d={username:t,amountBet:o,eventName:p.event,eventTime:p.eventTime,betType:p.betType,selectedTeam:p.selectedTeam,odds:p.odds,line:p.line,proofImageUrl:n,proofImageFilename:r,adminUsername:y};try{const l=await fetch(`${u}/api/submit-sports-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)});if(l.ok)document.getElementById("bettingSuccess").textContent="✅ Bet submitted successfully! Data saved to database.",document.getElementById("bettingError").textContent="",document.getElementById("bettingForm").reset(),setTimeout(()=>{F()},3e3);else{const i=await l.json();document.getElementById("bettingError").textContent=i.message||"Failed to place bet.",document.getElementById("bettingSuccess").textContent=""}}catch{document.getElementById("bettingError").textContent="Connection failed. Please try again.",document.getElementById("bettingSuccess").textContent=""}}window.toggleEventCard=function(e){console.log("Toggle called for index:",e);const t=document.getElementById(`event-content-${e}`),o=t.closest(".event-card");console.log("Content element:",t),console.log("Current display style:",t.style.display),t.style.display==="none"?(t.style.display="block",o.classList.remove("collapsed"),console.log("Expanding card")):(t.style.display="none",o.classList.add("collapsed"),console.log("Collapsing card"))};B?h():M();
