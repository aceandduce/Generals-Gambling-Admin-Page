(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&a(d)}).observe(document,{childList:!0,subtree:!0});function o(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=o(n);fetch(n.href,r)}})();let B=!1,g="",f="menu";const p="https://admin-site-ze7d.onrender.com",S="df860f2526805007d06289b53d901e26",P="https://api.the-odds-api.com/v4";function k(e){let t=e.value.replace(/[^\d]/g,"");if(!t){e.value="";return}t=parseInt(t,10).toString(),e.value=t.replace(/\B(?=(\d{3})+(?!\d))/g,","),e.value!==""&&(e.value="$"+e.value)}function x(e){return e?e.replace(/[^\d.\-]/g,""):""}function N(e){let t=e.value.replace(/[^\d]/g,"");t.length>10&&(t=t.slice(0,10));let o=t;t.length>6?o=t.slice(0,3)+"-"+t.slice(3,6)+"-"+t.slice(6):t.length>3&&(o=t.slice(0,3)+"-"+t.slice(3)),e.value=o}function M(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("addFundsBtn").onclick=()=>{f="addFunds",_()},document.getElementById("sportsBetsBtn").onclick=()=>{f="sportsBets",q()},document.getElementById("raffleTicketsBtn").onclick=()=>{f="raffleTickets",z()},document.getElementById("propBetsBtn").onclick=()=>{f="propBets",W()},document.getElementById("logoutBtn").onclick=()=>{B=!1,g="",f="menu",M()},document.getElementById("swapSheetBtn").onclick=O,fetch(`${p}/api/players`).then(e=>e.json()).then(e=>{const t=document.getElementById("playerList");t.innerHTML=e.map(o=>`<option value="${o.username}"></option>`).join("")}),document.getElementById("setActivePlayerBtn").onclick=async()=>{const e=document.getElementById("playerSelect").value.trim(),t=document.getElementById("activePlayerStatus");if(!e){t.innerHTML='<span style="color: orange;">Please select a player first.</span>';return}t.innerHTML='<span style="color: blue;">Setting active player...</span>';try{const o=await fetch(`${p}/api/set-active-player`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({playerName:e,adminUsername:g})}),a=await o.json();o.ok&&a.success?t.innerHTML=`<span style="color: green;">✓ Active player set to ${e}!</span>`:t.innerHTML='<span style="color: red;">✗ Failed to set active player.</span>'}catch{t.innerHTML='<span style="color: red;">✗ Error setting active player.</span>'}setTimeout(()=>{t.innerHTML=""},3e3)}}async function O(){const e=document.getElementById("sheetSelect").value,t=document.getElementById("swapStatus");if(!e){t.innerHTML='<span style="color: orange;">Please select a sheet first.</span>';return}t.innerHTML='<span style="color: blue;">Swapping sheets...</span>';try{const o=await fetch(`${p}/api/swap-sheet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({targetSheet:e,adminUsername:g})}),a=await o.json();o.ok?a.success?t.innerHTML=`<span style="color: green;">✓ Successfully swapped to ${e} sheet!</span>`:(await E(e),t.innerHTML=`<span style="color: green;">✓ Successfully swapped to ${e} sheet (fallback method)!</span>`):(await E(e),t.innerHTML=`<span style="color: green;">✓ Successfully swapped to ${e} sheet (fallback method)!</span>`)}catch(o){console.error("Sheet swap error:",o);try{await E(e),t.innerHTML=`<span style="color: green;">✓ Successfully swapped to ${e} sheet (fallback method)!</span>`}catch{t.innerHTML='<span style="color: red;">✗ Failed to swap sheets. Please try again.</span>'}}setTimeout(()=>{t.innerHTML=""},3e3)}async function E(e){const t=await fetch(`${p}/api/swap-sheet-fallback`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({targetSheet:e,adminUsername:g})});if(!t.ok)throw new Error("Fallback sheet swap failed");return await t.json()}function q(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("propAddToBetForm").onsubmit=a=>V(a,e,t)}async function V(e,t,o){e.preventDefault();const a=document.getElementById("propAddPlayers").value.trim(),n=Array.from(document.getElementById("propAddProofImages").files),r=a.split(",").map(i=>i.trim()).filter(Boolean);if(!r.length||!n.length){document.getElementById("propAddToBetError").innerText="All fields required.";return}document.getElementById("propAddToBetError").innerText="",document.getElementById("propAddToBetSuccess").innerText="Uploading images...";let d=[];try{for(const i of n){const c=await J(i);d.push(c)}}catch(i){document.getElementById("propAddToBetError").innerText=i.message,document.getElementById("propAddToBetSuccess").innerText="";return}const l=await G();for(const i of r)l.some(s=>s.username===i)&&await R(i,t.amount);await fetch(`${p}/api/join-prop-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({betID:t.betID,addPlayers:r.join(","),addProofUrls:d})}),document.getElementById("propAddToBetSuccess").innerText="Added to bet!",document.getElementById("propAddToBetError").innerText="",document.getElementById("propAddToBetForm").reset(),setTimeout(()=>o.remove(),2e3),T()}async function X(){try{const e=await fetch(`${P}/sports?apiKey=${S}`);if(!e.ok)throw new Error(`HTTP error! status: ${e.status}`);const t=await e.json(),o=document.getElementById("sportSelect");o.innerHTML='<option value="">Select a sport...</option>',t.filter(n=>n.active).sort((n,r)=>n.group===r.group?n.title.localeCompare(r.title):n.group.localeCompare(r.group)).forEach(n=>{const r=document.createElement("option");r.value=n.key,r.textContent=`${n.group} - ${n.title}`,o.appendChild(r)})}catch(e){console.error("Error loading sports:",e),document.getElementById("sportSelect").innerHTML='<option value="">Error loading sports</option>'}}async function ee(e){const t=document.getElementById("oddsContainer");t.innerHTML="<p>Loading odds...</p>";try{const o=await fetch(`${P}/sports/${e}/odds?regions=us&oddsFormat=decimal&markets=h2h,spreads,totals&apiKey=${S}`);if(!o.ok)throw new Error(`HTTP error! status: ${o.status}`);const a=await o.json(),n={title:te(e),events:a};oe(n)}catch(o){console.error("Error loading odds:",o),t.innerHTML="<p>Error loading odds. Please try again.</p>"}}function te(e){return{americanfootball_nfl:"NFL",americanfootball_ncaaf:"NCAAF",basketball_nba:"NBA",basketball_wnba:"WNBA",baseball_mlb:"MLB",icehockey_nhl:"NHL",soccer_epl:"EPL",soccer_usa_mls:"MLS",mma_mixed_martial_arts:"MMA"}[e]||e}function oe(e){const t=document.getElementById("oddsContainer");if(!e.events||e.events.length===0){t.innerHTML="<p>No events available for this sport at the moment.</p>";return}let o=`<h3>${e.title} - Upcoming Games</h3>`;e.events.forEach((r,d)=>{const l=new Date(r.commence_time).toLocaleString();o+=`
    ...
    `,r.bookmakers.forEach(i=>{o+=`
      ...
      `,i.markets.forEach(c=>{c.outcomes.forEach(s=>{const m=s.price;o+=`
            <tr>
              <td>${w(c.key)}</td>
              <td class="betting-cell" data-event="${r.away_team} @ ${r.home_team}" data-event-time="${r.commence_time}" data-bet-type="${w(c.key)}" data-selection="${s.name}" data-odds="${m.toFixed(2)}" data-line="${s.point?s.point:"-"}">${s.name}</td>
              <td>${m.toFixed(2)}</td>
