(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const n of e)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function r(e){const n={};return e.integrity&&(n.integrity=e.integrity),e.referrerPolicy&&(n.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?n.credentials="include":e.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function a(e){if(e.ep)return;e.ep=!0;const n=r(e);fetch(e.href,n)}})();let v=!1,g="",p="menu";const y="https://admin-site-ze7d.onrender.com",B="df860f2526805007d06289b53d901e26",I="https://api.the-odds-api.com/v4";function T(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("backToMenuBtn").onclick=()=>{p="menu",b()},document.getElementById("propExampleBtn").onclick=()=>{document.getElementById("propExampleModal").style.display="flex"},document.getElementById("closePropExampleModal").onclick=()=>{document.getElementById("propExampleModal").style.display="none"},document.getElementById("propBetForm").onsubmit=_}async function _(t){t.preventDefault();const o=document.getElementById("propPlayers").value.trim(),r=document.getElementById("propBetName").value.trim(),a=document.getElementById("propHowToWin").value.trim(),e=parseFloat(document.getElementById("propAmount").value),n=document.getElementById("propPasscode").value.trim(),i=Array.from(document.getElementById("propProofImages").files),l=o.split(",").map(m=>m.trim()).filter(Boolean);if(!l.length||!r||!a||!e||!n||!i.length){document.getElementById("propFormError").innerText="All fields are required.";return}document.getElementById("propFormError").innerText="",document.getElementById("propFormSuccess").innerText="Uploading images...";let s=[];const d=A();try{for(let m=0;m<i.length;m++){const M=i[m],L=`${d}_${m+1}`,{url:C,filename:D}=await E(M,L);s.push({url:C,filename:D})}}catch(m){document.getElementById("propFormError").innerText=m.message,document.getElementById("propFormSuccess").innerText="";return}const c=Math.round(e*.1*100)/100,f=Math.round((e*l.length+c)*100)/100,k=new Date().toLocaleString(),S=s.map(m=>m.url),P=s.map(m=>m.filename),F=[d,r,a,e,c,n,l.join(","),k,f,"active",...S,...P];await fetch(`${y}/api/submit-prop-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({row:F})}),document.getElementById("propFormSuccess").innerText="Prop bet submitted!",document.getElementById("propFormError").innerText="",document.getElementById("propBetForm").reset()}async function j(){try{const t=await fetch(`${I}/sports?apiKey=${B}`);if(!t.ok)throw new Error(`HTTP error! status: ${t.status}`);const o=await t.json(),r=document.getElementById("sportSelect");r.innerHTML='<option value="">Select a sport...</option>',o.filter(e=>e.active).sort((e,n)=>e.group===n.group?e.title.localeCompare(n.title):e.group.localeCompare(n.group)).forEach(e=>{const n=document.createElement("option");n.value=e.key,n.textContent=`${e.group} - ${e.title}`,r.appendChild(n)})}catch(t){console.error("Error loading sports:",t),document.getElementById("sportSelect").innerHTML='<option value="">Error loading sports</option>'}}async function z(t){const o=document.getElementById("oddsContainer");o.innerHTML="<p>Loading odds...</p>";try{const r=await fetch(`${I}/sports/${t}/odds?regions=us&oddsFormat=decimal&markets=h2h,spreads,totals&apiKey=${B}`);if(!r.ok)throw new Error(`HTTP error! status: ${r.status}`);const a=await r.json(),e={title:G(t),events:a};R(e)}catch(r){console.error("Error loading odds:",r),o.innerHTML="<p>Error loading odds. Please try again.</p>"}}function G(t){return{americanfootball_nfl:"NFL",americanfootball_ncaaf:"NCAAF",basketball_nba:"NBA",basketball_wnba:"WNBA",baseball_mlb:"MLB",icehockey_nhl:"NHL",soccer_epl:"EPL",soccer_usa_mls:"MLS",mma_mixed_martial_arts:"MMA"}[t]||t}function R(t){const o=document.getElementById("oddsContainer");if(!t.events||t.events.length===0){o.innerHTML="<p>No events available for this sport at the moment.</p>";return}let r=`<h3>${t.title} - Upcoming Games</h3>`;t.events.forEach((e,n)=>{const i=new Date(e.commence_time).toLocaleString();r+=`
    ...
    `,e.bookmakers.forEach(l=>{r+=`
        <div class="bookmaker-section">
          ...
          <table class="odds-table">
            ...
            <tbody>
      `,l.markets.forEach(s=>{s.outcomes.forEach(d=>{const c=d.price;r+=`
            <tr>
              <td>${x(s.key)}</td>
  ...
  `,t.events.forEach((e,n)=>{const i=document.getElementById(`event-card-${n}`),l=document.getElementById(`event-content-${n}`);i&&l&&(l.style.display="none",i.classList.add("collapsed"),i.addEventListener("click",function(s){s.target.closest(".event-content")||(l.style.display==="none"?(l.style.display="block",i.classList.remove("collapsed")):(l.style.display="none",i.classList.add("collapsed")))}))}),document.querySelectorAll(".betting-cell").forEach(e=>{e.addEventListener("click",function(n){n.stopPropagation(),W(this)})}),document.getElementById("closeBettingModal").addEventListener("click",w),document.getElementById("closeBettingExampleModal").addEventListener("click",Y),document.getElementById("bettingExampleBtn").addEventListener("click",()=>{document.getElementById("bettingExampleModal").style.display="flex"});const a=document.getElementById("bettingProofImage");document.getElementById("bettingForm").addEventListener("paste",e=>{if(e.clipboardData&&e.clipboardData.files&&e.clipboardData.files.length>0){const n=e.clipboardData.files[0];if(n.type.startsWith("image/")){const i=new DataTransfer;i.items.add(n),a.files=i.files}}}),document.getElementById("bettingForm").addEventListener("submit",J)}function x(t){return{h2h:"Moneyline",spreads:"Spread",totals:"Total"}[t]||t}let u=null;function W(t){const o=t.dataset.event,r=t.dataset.eventTime,a=t.dataset.betType,e=t.dataset.selection,n=t.dataset.odds,i=t.dataset.line;u={event:o,eventTime:r,betType:a,selection:e,odds:n,line:i},document.getElementById("modalEventTitle").textContent=`${o} - ${a}`;const l=document.getElementById("teamSelection");if(l.innerHTML="",a==="Moneyline")o.split(" @ ").forEach(d=>{const c=document.createElement("button");c.type="button",c.textContent=d,c.style.cssText="padding:0.5rem 1rem; background:#333; color:#fff; border:1px solid #444; border-radius:5px; cursor:pointer; flex:1;",c.addEventListener("click",function(){l.querySelectorAll("button").forEach(f=>{f.style.background="#333",f.style.borderColor="#444"}),this.style.background="#646cff",this.style.borderColor="#646cff",u.selectedTeam=this.textContent}),l.appendChild(c)});else{const s=document.createElement("button");s.type="button",s.textContent=e,s.style.cssText="padding:0.5rem 1rem; background:#646cff; color:#fff; border:1px solid #646cff; border-radius:5px; cursor:pointer; width:100%;",u.selectedTeam=e,l.appendChild(s)}document.getElementById("bettingForm").reset(),document.getElementById("bettingError").textContent="",document.getElementById("bettingSuccess").textContent="",document.getElementById("bettingModal").style.display="flex"}function w(){document.getElementById("bettingModal").style.display="none",u=null}function Y(){document.getElementById("bettingExampleModal").style.display="none"}async function J(t){if(t.preventDefault(),!u||!u.selectedTeam){document.getElementById("bettingError").textContent="Please select a team.";return}const o=document.getElementById("bettingUsername").value,r=document.getElementById("bettingAmount").value,a=document.getElementById("bettingProofImage").files[0];if(!a){document.getElementById("bettingError").textContent="Image required.";return}document.getElementById("bettingSuccess").textContent="Processing... Please wait.",document.getElementById("bettingError").textContent="";let e="",n="";try{const l=h(12),{url:s,filename:d}=await E(a,l);e=s,n=d}catch(l){document.getElementById("bettingError").textContent=l.message,document.getElementById("bettingSuccess").textContent="";return}const i={username:o,amountBet:r,eventName:u.event,eventTime:u.eventTime,betType:u.betType,selectedTeam:u.selectedTeam,odds:u.odds,line:u.line,proofImageUrl:e,proofImageFilename:n,adminUsername:g};try{const l=await fetch(`${y}/api/submit-sports-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(l.ok)document.getElementById("bettingSuccess").textContent="✅ Bet submitted successfully! Data saved to database.",document.getElementById("bettingError").textContent="",document.getElementById("bettingForm").reset(),setTimeout(()=>{w()},3e3);else{const s=await l.json();document.getElementById("bettingError").textContent=s.message||"Failed to place bet.",document.getElementById("bettingSuccess").textContent=""}}catch{document.getElementById("bettingError").textContent="Connection failed. Please try again.",document.getElementById("bettingSuccess").textContent=""}}window.toggleEventCard=function(t){console.log("Toggle called for index:",t);const o=document.getElementById(`event-content-${t}`),r=o.closest(".event-card");console.log("Content element:",o),console.log("Current display style:",o.style.display),o.style.display==="none"?(o.style.display="block",r.classList.remove("collapsed"),console.log("Expanding card")):(o.style.display="none",r.classList.add("collapsed"),console.log("Collapsing card"))};v?b():T();
