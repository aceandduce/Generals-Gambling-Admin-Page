(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&a(d)}).observe(document,{childList:!0,subtree:!0});function o(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(n){if(n.ep)return;n.ep=!0;const r=o(n);fetch(n.href,r)}})();let B=!1,y="",f="menu";const u="https://admin-site-ze7d.onrender.com",S="df860f2526805007d06289b53d901e26",P="https://api.the-odds-api.com/v4";function k(e){let t=e.value.replace(/[^\d]/g,"");if(!t){e.value="";return}t=parseInt(t,10).toString(),e.value=t.replace(/\B(?=(\d{3})+(?!\d))/g,","),e.value!==""&&(e.value="$"+e.value)}function x(e){return e?e.replace(/[^\d.\-]/g,""):""}function N(e){let t=e.value.replace(/[^\d]/g,"");t.length>10&&(t=t.slice(0,10));let o=t;t.length>6?o=t.slice(0,3)+"-"+t.slice(3,6)+"-"+t.slice(6):t.length>3&&(o=t.slice(0,3)+"-"+t.slice(3)),e.value=o}function M(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("backToMenuBtn").onclick=()=>{f="menu",b()},document.getElementById("propExampleBtn").onclick=()=>{document.getElementById("propExampleModal").style.display="flex"},document.getElementById("closePropExampleModal").onclick=()=>{document.getElementById("propExampleModal").style.display="none"},document.getElementById("propBetForm").onsubmit=Y,w()}async function R(){const e=await fetch(`${u}/api/players`);return e.ok?await e.json():[]}async function G(e,t){await fetch(`${u}/api/deduct-balance`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:e,amount:t})})}async function Y(e){e.preventDefault();const t=document.getElementById("propPlayers").value.trim(),o=document.getElementById("propBetName").value.trim(),a=document.getElementById("propHowToWin").value.trim(),n=parseFloat(x(document.getElementById("propAmount").value)),r=document.getElementById("propPasscode").value.trim(),d=Array.from(document.getElementById("propProofImages").files),l=t.split(",").map(g=>g.trim()).filter(Boolean);if(!l.length||!o||!a||!n||!r||!d.length){document.getElementById("propFormError").innerText="All fields are required.";return}document.getElementById("propFormError").innerText="",document.getElementById("propFormSuccess").innerText="Uploading images...";let i=[];try{for(const g of d){const{url:D,filename:H}=await h(g);i.push({url:D,filename:H})}}catch(g){document.getElementById("propFormError").innerText=g.message,document.getElementById("propFormSuccess").innerText="";return}const c=U(),s=Math.round(n*.1*100)/100,m=Math.round((n*l.length+s)*100)/100,v=new Date().toLocaleString(),A=i.map(g=>g.url),$=i.map(g=>g.filename),C=[c,o,a,n,s,r,l.join(","),v,m,"active",...A,...$];await fetch(`${u}/api/submit-prop-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({row:C})}),document.getElementById("propFormSuccess").innerText="Prop bet submitted!",document.getElementById("propFormError").innerText="",document.getElementById("propBetForm").reset(),w()}async function w(){const e=await fetch(`${u}/api/active-prop-bets`),t=e.ok?await e.json():[],o=document.getElementById("propActiveDropdownContainer");o.innerHTML=`
