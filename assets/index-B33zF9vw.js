(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const o of e)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function r(e){const o={};return e.integrity&&(o.integrity=e.integrity),e.referrerPolicy&&(o.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?o.credentials="include":e.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(e){if(e.ep)return;e.ep=!0;const o=r(e);fetch(e.href,o)}})();let b=!1,p="",u="menu";const y="https://admin-site-ze7d.onrender.com",v="df860f2526805007d06289b53d901e26",E="https://api.the-odds-api.com/v4";function x(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("addFundsBtn").onclick=()=>{u="addFunds",T()},document.getElementById("sportsBetsBtn").onclick=()=>{u="sportsBets",I()},document.getElementById("raffleTicketsBtn").onclick=()=>{u="raffleTickets",k()},document.getElementById("logoutBtn").onclick=()=>{b=!1,p="",u="menu",x()}}function I(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("backToMenuBtn").onclick=()=>{u="menu",g()},w(),document.getElementById("sportSelect").addEventListener("change",function(){const n=this.value;n?S(n):document.getElementById("oddsContainer").innerHTML="<p>Select a sport to view available odds</p>"})}function T(){document.querySelector("#app").innerHTML=`
    <div style="display: flex; flex-direction: column; align-items: center; gap: 1rem;">
      <button id="backToMenuBtn" style="padding: 0.5rem 1rem; background: #666; color: #fff; border: none; border-radius: 5px; font-size: 0.9rem; cursor: pointer; width: auto;">← Back to Menu</button>
      <div class="form-container">
  ...
  `,document.getElementById("raffleExampleBtn").onclick=()=>{document.getElementById("raffleExampleModal").style.display="flex"},document.getElementById("closeRaffleExampleModal").onclick=()=>{document.getElementById("raffleExampleModal").style.display="none"},document.getElementById("backToMenuBtn").onclick=()=>{u="menu",g()};const n=document.getElementById("raffleProofImage");document.getElementById("raffleForm").addEventListener("paste",t=>{if(t.clipboardData&&t.clipboardData.files&&t.clipboardData.files.length>0){const r=t.clipboardData.files[0];if(r.type.startsWith("image/")){const a=new DataTransfer;a.items.add(r),n.files=a.files}}}),document.getElementById("raffleForm").onsubmit=async t=>{t.preventDefault();const r=document.getElementById("phoneNumber").value,a=document.getElementById("stateId").value,e=document.getElementById("amountPurchased").value,o=document.getElementById("raffleProofImage").files[0];if(!o){document.getElementById("raffleFormError").innerText="Image required.";return}const l="tAhG8ZNH6lBSEf0xnJT2aOuDP7Jiu9u7";let i="";try{const d=new FormData;d.append("file",o);const c=await fetch("https://api.fivemanage.com/v1/images",{method:"POST",headers:{Authorization:`Bearer ${l}`},body:d}),f=await c.json();if(c.ok)i=f.url;else{document.getElementById("raffleFormError").innerText="Image upload failed.";return}}catch{document.getElementById("raffleFormError").innerText="Image upload failed.";return}const s={phoneNumber:r,stateId:a,amountPurchased:e,proofImageUrl:i,adminUsername:p};try{const d=await fetch(`${y}/api/submit-raffle-ticket`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(d.ok)document.getElementById("raffleFormSuccess").innerText="Raffle ticket submitted successfully!",document.getElementById("raffleFormError").innerText="",document.getElementById("raffleForm").reset();else{const c=await d.json();document.getElementById("raffleFormError").innerText=c.message||"Submission failed.",document.getElementById("raffleFormSuccess").innerText=""}}catch{document.getElementById("raffleFormError").innerText="Connection failed. Please try again.",document.getElementById("raffleFormSuccess").innerText=""}}}async function w(){try{const n=await fetch(`${E}/sports?apiKey=${v}`);if(!n.ok)throw new Error(`HTTP error! status: ${n.status}`);const t=await n.json(),r=document.getElementById("sportSelect");r.innerHTML='<option value="">Select a sport...</option>',t.filter(e=>e.active).sort((e,o)=>e.group===o.group?e.title.localeCompare(o.title):e.group.localeCompare(o.group)).forEach(e=>{const o=document.createElement("option");o.value=e.key,o.textContent=`${e.group} - ${e.title}`,r.appendChild(o)})}catch(n){console.error("Error loading sports:",n),document.getElementById("sportSelect").innerHTML='<option value="">Error loading sports</option>'}}async function S(n){const t=document.getElementById("oddsContainer");t.innerHTML="<p>Loading odds...</p>";try{const r=await fetch(`${E}/sports/${n}/odds?regions=us&oddsFormat=decimal&markets=h2h,spreads,totals&apiKey=${v}`);if(!r.ok)throw new Error(`HTTP error! status: ${r.status}`);const a=await r.json(),e={title:M(n),events:a};P(e)}catch(r){console.error("Error loading odds:",r),t.innerHTML="<p>Error loading odds. Please try again.</p>"}}function M(n){return{americanfootball_nfl:"NFL",americanfootball_ncaaf:"NCAAF",basketball_nba:"NBA",basketball_wnba:"WNBA",baseball_mlb:"MLB",icehockey_nhl:"NHL",soccer_epl:"EPL",soccer_usa_mls:"MLS",mma_mixed_martial_arts:"MMA"}[n]||n}function P(n){const t=document.getElementById("oddsContainer");if(!n.events||n.events.length===0){t.innerHTML="<p>No events available for this sport at the moment.</p>";return}let r=`<h3>${n.title} - Upcoming Games</h3>`;n.events.forEach((e,o)=>{const l=new Date(e.commence_time).toLocaleString();r+=`
    ...
    `,e.bookmakers.forEach(i=>{r+=`
      ...
      `,i.markets.forEach(s=>{s.outcomes.forEach(d=>{const c=d.price;r+=`
            <tr>
              <td>${h(s.key)}</td>
              <td class="betting-cell" data-event="${e.away_team} @ ${e.home_team}" data-event-time="${e.commence_time}" data-bet-type="${h(s.key)}" data-selection="${d.name}" data-odds="${c.toFixed(2)}" data-line="${d.point?d.point:"-"}">${d.name}</td>
              <td>${c.toFixed(2)}</td>
  ...
  `,n.events.forEach((e,o)=>{const l=document.getElementById(`event-card-${o}`),i=document.getElementById(`event-content-${o}`);l&&i&&(i.style.display="none",l.classList.add("collapsed"),l.addEventListener("click",function(s){s.target.closest(".event-content")||(i.style.display==="none"?(i.style.display="block",l.classList.remove("collapsed")):(i.style.display="none",l.classList.add("collapsed")))}))}),document.querySelectorAll(".betting-cell").forEach(e=>{e.addEventListener("click",function(o){o.stopPropagation(),L(this)})}),document.getElementById("closeBettingModal").addEventListener("click",B),document.getElementById("closeBettingExampleModal").addEventListener("click",C),document.getElementById("bettingExampleBtn").addEventListener("click",()=>{document.getElementById("bettingExampleModal").style.display="flex"});const a=document.getElementById("bettingProofImage");document.getElementById("bettingForm").addEventListener("paste",e=>{if(e.clipboardData&&e.clipboardData.files&&e.clipboardData.files.length>0){const o=e.clipboardData.files[0];if(o.type.startsWith("image/")){const l=new DataTransfer;l.items.add(o),a.files=l.files}}}),document.getElementById("bettingForm").addEventListener("submit",F)}function h(n){return{h2h:"Moneyline",spreads:"Spread",totals:"Total"}[n]||n}let m=null;function L(n){const t=n.dataset.event,r=n.dataset.eventTime,a=n.dataset.betType,e=n.dataset.selection,o=n.dataset.odds,l=n.dataset.line;m={event:t,eventTime:r,betType:a,selection:e,odds:o,line:l},document.getElementById("modalEventTitle").textContent=`${t} - ${a}`;const i=document.getElementById("teamSelection");if(i.innerHTML="",a==="Moneyline")t.split(" @ ").forEach(d=>{const c=document.createElement("button");c.type="button",c.textContent=d,c.style.cssText="padding:0.5rem 1rem; background:#333; color:#fff; border:1px solid #444; border-radius:5px; cursor:pointer; flex:1;",c.addEventListener("click",function(){i.querySelectorAll("button").forEach(f=>{f.style.background="#333",f.style.borderColor="#444"}),this.style.background="#646cff",this.style.borderColor="#646cff",m.selectedTeam=this.textContent}),i.appendChild(c)});else{const s=document.createElement("button");s.type="button",s.textContent=e,s.style.cssText="padding:0.5rem 1rem; background:#646cff; color:#fff; border:1px solid #646cff; border-radius:5px; cursor:pointer; width:100%;",m.selectedTeam=e,i.appendChild(s)}document.getElementById("bettingForm").reset(),document.getElementById("bettingError").textContent="",document.getElementById("bettingSuccess").textContent="",document.getElementById("bettingModal").style.display="flex"}function B(){document.getElementById("bettingModal").style.display="none",m=null}function C(){document.getElementById("bettingExampleModal").style.display="none"}async function F(n){if(n.preventDefault(),!m||!m.selectedTeam){document.getElementById("bettingError").textContent="Please select a team.";return}const t=document.getElementById("bettingUsername").value,r=document.getElementById("bettingAmount").value,a=document.getElementById("bettingProofImage").files[0];if(!a){document.getElementById("bettingError").textContent="Image required.";return}document.getElementById("bettingSuccess").textContent="Processing... Please wait.",document.getElementById("bettingError").textContent="";const e="tAhG8ZNH6lBSEf0xnJT2aOuDP7Jiu9u7";let o="";try{const i=new FormData;i.append("file",a);const s=await fetch("https://api.fivemanage.com/v1/images",{method:"POST",headers:{Authorization:`Bearer ${e}`},body:i}),d=await s.json();if(s.ok)o=d.url;else{document.getElementById("bettingError").textContent="Image upload failed.",document.getElementById("bettingSuccess").textContent="";return}}catch{document.getElementById("bettingError").textContent="Image upload failed.",document.getElementById("bettingSuccess").textContent="";return}const l={username:t,amountBet:r,eventName:m.event,eventTime:m.eventTime,betType:m.betType,selectedTeam:m.selectedTeam,odds:m.odds,line:m.line,proofImageUrl:o,adminUsername:p};try{const i=await fetch(`${y}/api/submit-sports-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)});if(i.ok)document.getElementById("bettingSuccess").textContent="✅ Bet submitted successfully! Data saved to database.",document.getElementById("bettingError").textContent="",document.getElementById("bettingForm").reset(),setTimeout(()=>{B()},3e3);else{const s=await i.json();document.getElementById("bettingError").textContent=s.message||"Failed to place bet.",document.getElementById("bettingSuccess").textContent=""}}catch{document.getElementById("bettingError").textContent="Connection failed. Please try again.",document.getElementById("bettingSuccess").textContent=""}}window.toggleEventCard=function(n){console.log("Toggle called for index:",n);const t=document.getElementById(`event-content-${n}`),r=t.closest(".event-card");console.log("Content element:",t),console.log("Current display style:",t.style.display),t.style.display==="none"?(t.style.display="block",r.classList.remove("collapsed"),console.log("Expanding card")):(t.style.display="none",r.classList.add("collapsed"),console.log("Collapsing card"))};b?g():x();
