(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function n(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(o){if(o.ep)return;o.ep=!0;const r=n(o);fetch(o.href,r)}})();let v=!1,g="",p="menu";const f="https://admin-site-ze7d.onrender.com",I="df860f2526805007d06289b53d901e26",T="https://api.the-odds-api.com/v4";function w(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("loginForm").onsubmit=async e=>{e.preventDefault();const t=document.getElementById("loginUsername").value,n=document.getElementById("loginPassword").value;document.getElementById("loginLoading").style.display="block",document.getElementById("loginError").innerText="";try{const a=await fetch(`${f}/api/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:t,password:n})});if(a.ok)v=!0,g=t,p="menu",y();else{const o=await a.json();document.getElementById("loginError").innerText=o.message||"Login failed"}}catch{document.getElementById("loginError").innerText="Connection failed. Please try again."}finally{document.getElementById("loginLoading").style.display="none"}}}function E(e=8){const t="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";let n="";for(let a=0;a<e;a++)n+=t.charAt(Math.floor(Math.random()*t.length));return n}function P(e){return e.split(".").pop()}function C(){return"PB-"+E(10)}function $(e){const t=P(e.name);return`GeneralsGamblingAdmin_${E(12)}.${t}`}async function h(e){const t="tAhG8ZNH6lBSEf0xnJT2aOuDP7Jiu9u7",n=$(e),a=new FormData;a.append("file",e,n),a.append("metadata",JSON.stringify({name:n}));const o=await fetch("https://fmapi.net/api/v2/image",{method:"POST",headers:{Authorization:t},body:a}),r=await o.json();if(o.ok&&r.data&&r.data.url)return{url:r.data.url,filename:n};throw new Error(r?.message||"Image upload failed.")}async function N(e){const t="tAhG8ZNH6lBSEf0xnJT2aOuDP7Jiu9u7",n=new FormData,a=P(e.name),o=`GeneralsGamblingAdmin_${E(12)}.${a}`;n.append("file",e,o),n.append("metadata",JSON.stringify({name:o}));const r=await fetch("https://fmapi.net/api/v2/image",{method:"POST",headers:{Authorization:t},body:n}),i=await r.json();if(r.ok&&i.data&&i.data.url)return i.data.url;throw new Error(i?.message||"Image upload failed.")}function y(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("addFundsBtn").onclick=()=>{p="addFunds",U()},document.getElementById("sportsBetsBtn").onclick=()=>{p="sportsBets",q()},document.getElementById("raffleTicketsBtn").onclick=()=>{p="raffleTickets",J()},document.getElementById("propBetsBtn").onclick=()=>{p="propBets",H()},document.getElementById("logoutBtn").onclick=()=>{v=!1,g="",p="menu",w()}}function q(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("backToMenuBtn").onclick=()=>{p="menu",y()},Y(),document.getElementById("sportSelect").addEventListener("change",function(){const e=this.value;e?K(e):document.getElementById("oddsContainer").innerHTML="<p>Select a sport to view available odds</p>"})}function U(){document.querySelector("#app").innerHTML=`
    <div style="display: flex; flex-direction: column; align-items: center; gap: 1rem;">
      <button id="backToMenuBtn" style="padding: 0.5rem 1rem; background: #666; color: #fff; border: none; border-radius: 5px; font-size: 0.9rem; cursor: pointer; width: auto;">← Back to Menu</button>
      <div class="form-container">
  ...
  `,document.getElementById("backToMenuBtn").onclick=()=>{p="menu",y()},document.getElementById("propExampleBtn").onclick=()=>{document.getElementById("propExampleModal").style.display="flex"},document.getElementById("closePropExampleModal").onclick=()=>{document.getElementById("propExampleModal").style.display="none"},document.getElementById("propBetForm").onsubmit=_,B()}async function O(){const e=await fetch(`${f}/api/players`);return e.ok?await e.json():[]}async function j(e,t){await fetch(`${f}/api/deduct-balance`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:e,amount:t})})}async function _(e){e.preventDefault();const t=document.getElementById("propPlayers").value.trim(),n=document.getElementById("propBetName").value.trim(),a=document.getElementById("propHowToWin").value.trim(),o=parseFloat(document.getElementById("propAmount").value),r=document.getElementById("propPasscode").value.trim(),i=Array.from(document.getElementById("propProofImages").files),l=t.split(",").map(u=>u.trim()).filter(Boolean);if(!l.length||!n||!a||!o||!r||!i.length){document.getElementById("propFormError").innerText="All fields are required.";return}document.getElementById("propFormError").innerText="",document.getElementById("propFormSuccess").innerText="Uploading images...";let d=[];try{for(const u of i){const{url:L,filename:D}=await h(u);d.push({url:L,filename:D})}}catch(u){document.getElementById("propFormError").innerText=u.message,document.getElementById("propFormSuccess").innerText="";return}const s=C(),c=Math.round(o*.1*100)/100,b=Math.round((o*l.length+c)*100)/100,k=new Date().toLocaleString(),M=d.map(u=>u.url),F=d.map(u=>u.filename),A=[s,n,a,o,c,r,l.join(","),k,b,"active",...M,...F];await fetch(`${f}/api/submit-prop-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({row:A})}),document.getElementById("propFormSuccess").innerText="Prop bet submitted!",document.getElementById("propFormError").innerText="",document.getElementById("propBetForm").reset(),B()}async function B(){const e=await fetch(`${f}/api/active-prop-bets`),t=e.ok?await e.json():[],n=document.getElementById("propActiveDropdownContainer");n.innerHTML=`
  ...
  `,document.getElementById("propJoinBtn").onclick=()=>z(t)}async function z(e){const t=document.getElementById("propActiveDropdown").value.trim(),n=e.find(a=>a.betName===t);if(!n){document.getElementById("propJoinError").innerText="Bet not found.";return}G(n)}function G(e){let t=document.getElementById("propJoinModal");t&&t.remove(),t=document.createElement("div"),t.id="propJoinModal",t.style="position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.7); display:flex; align-items:center; justify-content:center; z-index:2000;",t.innerHTML=`
  ...
  `,document.body.appendChild(t),document.getElementById("closePropJoinModal").onclick=()=>t.remove(),document.getElementById("propJoinPassForm").onsubmit=n=>{if(n.preventDefault(),document.getElementById("propJoinPasscode").value!==e.passcode){document.getElementById("propJoinPassError").innerText="Incorrect passcode.";return}document.getElementById("propJoinPassError").innerText="",W(e,t)}}function W(e,t){const n=document.getElementById("propJoinDetails");n.style.display="block",n.innerHTML=`
  ...
  `,document.getElementById("propAddToBetForm").onsubmit=a=>R(a,e,t)}async function R(e,t,n){e.preventDefault();const a=document.getElementById("propAddPlayers").value.trim(),o=Array.from(document.getElementById("propAddProofImages").files),r=a.split(",").map(d=>d.trim()).filter(Boolean);if(!r.length||!o.length){document.getElementById("propAddToBetError").innerText="All fields required.";return}document.getElementById("propAddToBetError").innerText="",document.getElementById("propAddToBetSuccess").innerText="Uploading images...";let i=[];try{for(const d of o){const s=await N(d);i.push(s)}}catch(d){document.getElementById("propAddToBetError").innerText=d.message,document.getElementById("propAddToBetSuccess").innerText="";return}const l=await O();for(const d of r)l.some(c=>c.username===d)&&await j(d,t.amount);await fetch(`${f}/api/join-prop-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({betID:t.betID,addPlayers:r.join(","),addProofUrls:i})}),document.getElementById("propAddToBetSuccess").innerText="Added to bet!",document.getElementById("propAddToBetError").innerText="",document.getElementById("propAddToBetForm").reset(),setTimeout(()=>n.remove(),2e3),B()}async function Y(){try{const e=await fetch(`${T}/sports?apiKey=${I}`);if(!e.ok)throw new Error(`HTTP error! status: ${e.status}`);const t=await e.json(),n=document.getElementById("sportSelect");n.innerHTML='<option value="">Select a sport...</option>',t.filter(o=>o.active).sort((o,r)=>o.group===r.group?o.title.localeCompare(r.title):o.group.localeCompare(r.group)).forEach(o=>{const r=document.createElement("option");r.value=o.key,r.textContent=`${o.group} - ${o.title}`,n.appendChild(r)})}catch(e){console.error("Error loading sports:",e),document.getElementById("sportSelect").innerHTML='<option value="">Error loading sports</option>'}}async function K(e){const t=document.getElementById("oddsContainer");t.innerHTML="<p>Loading odds...</p>";try{const n=await fetch(`${T}/sports/${e}/odds?regions=us&oddsFormat=decimal&markets=h2h,spreads,totals&apiKey=${I}`);if(!n.ok)throw new Error(`HTTP error! status: ${n.status}`);const a=await n.json(),o={title:Z(e),events:a};Q(o)}catch(n){console.error("Error loading odds:",n),t.innerHTML="<p>Error loading odds. Please try again.</p>"}}function Z(e){return{americanfootball_nfl:"NFL",americanfootball_ncaaf:"NCAAF",basketball_nba:"NBA",basketball_wnba:"WNBA",baseball_mlb:"MLB",icehockey_nhl:"NHL",soccer_epl:"EPL",soccer_usa_mls:"MLS",mma_mixed_martial_arts:"MMA"}[e]||e}function Q(e){const t=document.getElementById("oddsContainer");if(!e.events||e.events.length===0){t.innerHTML="<p>No events available for this sport at the moment.</p>";return}let n=`<h3>${e.title} - Upcoming Games</h3>`;e.events.forEach((o,r)=>{const i=new Date(o.commence_time).toLocaleString();n+=`
  ...
  `,e.events.forEach((o,r)=>{const i=document.getElementById(`event-card-${r}`),l=document.getElementById(`event-content-${r}`);i&&l&&(l.style.display="none",i.classList.add("collapsed"),i.addEventListener("click",function(d){d.target.closest(".event-content")||(l.style.display==="none"?(l.style.display="block",i.classList.remove("collapsed")):(l.style.display="none",i.classList.add("collapsed")))}))}),document.querySelectorAll(".betting-cell").forEach(o=>{o.addEventListener("click",function(r){r.stopPropagation(),V(this)})}),document.getElementById("closeBettingModal").addEventListener("click",S),document.getElementById("closeBettingExampleModal").addEventListener("click",X),document.getElementById("bettingExampleBtn").addEventListener("click",()=>{document.getElementById("bettingExampleModal").style.display="flex"});const a=document.getElementById("bettingProofImage");document.getElementById("bettingForm").addEventListener("paste",o=>{if(o.clipboardData&&o.clipboardData.files&&o.clipboardData.files.length>0){const r=o.clipboardData.files[0];if(r.type.startsWith("image/")){const i=new DataTransfer;i.items.add(r),a.files=i.files}}}),document.getElementById("bettingForm").addEventListener("submit",ee)}function x(e){return{h2h:"Moneyline",spreads:"Spread",totals:"Total"}[e]||e}let m=null;function V(e){const t=e.dataset.event,n=e.dataset.eventTime,a=e.dataset.betType,o=e.dataset.selection,r=e.dataset.odds,i=e.dataset.line;m={event:t,eventTime:n,betType:a,selection:o,odds:r,line:i},document.getElementById("modalEventTitle").textContent=`${t} - ${a}`;const l=document.getElementById("teamSelection");if(l.innerHTML="",a==="Moneyline")t.split(" @ ").forEach(s=>{const c=document.createElement("button");c.type="button",c.textContent=s,c.style.cssText="padding:0.5rem 1rem; background:#333; color:#fff; border:1px solid #444; border-radius:5px; cursor:pointer; flex:1;",c.addEventListener("click",function(){l.querySelectorAll("button").forEach(b=>{b.style.background="#333",b.style.borderColor="#444"}),this.style.background="#646cff",this.style.borderColor="#646cff",m.selectedTeam=this.textContent}),l.appendChild(c)});else{const d=document.createElement("button");d.type="button",d.textContent=o,d.style.cssText="padding:0.5rem 1rem; background:#646cff; color:#fff; border:1px solid #646cff; border-radius:5px; cursor:pointer; width:100%;",m.selectedTeam=o,l.appendChild(d)}document.getElementById("bettingForm").reset(),document.getElementById("bettingError").textContent="",document.getElementById("bettingSuccess").textContent="",document.getElementById("bettingModal").style.display="flex"}function S(){document.getElementById("bettingModal").style.display="none",m=null}function X(){document.getElementById("bettingExampleModal").style.display="none"}async function ee(e){if(e.preventDefault(),!m||!m.selectedTeam){document.getElementById("bettingError").textContent="Please select a team.";return}const t=document.getElementById("bettingUsername").value,n=document.getElementById("bettingAmount").value,a=document.getElementById("bettingProofImage").files[0];if(!a){document.getElementById("bettingError").textContent="Image required.";return}document.getElementById("bettingSuccess").textContent="Processing... Please wait.",document.getElementById("bettingError").textContent="";let o="",r="";try{const{url:l,filename:d}=await h(a);o=l,r=d}catch(l){document.getElementById("bettingError").textContent=l.message,document.getElementById("bettingSuccess").textContent="";return}const i={username:t,amountBet:n,eventName:m.event,eventTime:m.eventTime,betType:m.betType,selectedTeam:m.selectedTeam,odds:m.odds,line:m.line,proofImageUrl:o,proofImageFilename:r,adminUsername:g};try{const l=await fetch(`${f}/api/submit-sports-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(l.ok)document.getElementById("bettingSuccess").textContent="✅ Bet submitted successfully! Data saved to database.",document.getElementById("bettingError").textContent="",document.getElementById("bettingForm").reset(),setTimeout(()=>{S()},3e3);else{const d=await l.json();document.getElementById("bettingError").textContent=d.message||"Failed to place bet.",document.getElementById("bettingSuccess").textContent=""}}catch{document.getElementById("bettingError").textContent="Connection failed. Please try again.",document.getElementById("bettingSuccess").textContent=""}}window.toggleEventCard=function(e){console.log("Toggle called for index:",e);const t=document.getElementById(`event-content-${e}`),n=t.closest(".event-card");console.log("Content element:",t),console.log("Current display style:",t.style.display),t.style.display==="none"?(t.style.display="block",n.classList.remove("collapsed"),console.log("Expanding card")):(t.style.display="none",n.classList.add("collapsed"),console.log("Collapsing card"))};v?y():w();
