(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))d(e);new MutationObserver(e=>{for(const n of e)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&d(i)}).observe(document,{childList:!0,subtree:!0});function r(e){const n={};return e.integrity&&(n.integrity=e.integrity),e.referrerPolicy&&(n.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?n.credentials="include":e.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function d(e){if(e.ep)return;e.ep=!0;const n=r(e);fetch(e.href,n)}})();let b=!1,p="";const y="https://admin-site-ze7d.onrender.com",v="df860f2526805007d06289b53d901e26",E="https://api.the-odds-api.com/v4";function x(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("loginForm").onsubmit=async t=>{t.preventDefault();const o=document.getElementById("loginUsername").value,r=document.getElementById("loginPassword").value,d=await fetch(`${y}/api/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:o,password:r})});if(d.ok)b=!0,p=o,f();else{const e=await d.json();document.getElementById("loginError").innerText=e.message||"Login failed"}}}function f(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("addFundsBtn").onclick=()=>{T()},document.getElementById("sportsBetsBtn").onclick=()=>{I()},document.getElementById("logoutBtn").onclick=()=>{b=!1,p="",x()}}function I(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("backToMenuBtn").onclick=()=>{f()},w(),document.getElementById("sportSelect").addEventListener("change",function(){const t=this.value;t?S(t):document.getElementById("oddsContainer").innerHTML="<p>Select a sport to view available odds</p>"})}function T(){document.querySelector("#app").innerHTML=`
    <div style="display: flex; flex-direction: column; align-items: center; gap: 1rem;">
      <button id="backToMenuBtn" style="padding: 0.5rem 1rem; background: #666; color: #fff; border: none; border-radius: 5px; font-size: 0.9rem; cursor: pointer; width: auto;">← Back to Menu</button>
      <div class="form-container">
    ...
    `,e.bookmakers.forEach(c=>{r+=`
        <div class="bookmaker-section">
          ...
          <table class="odds-table">
            ...
            <tbody>
      `,c.markets.forEach(a=>{a.outcomes.forEach(s=>{const l=s.price,u=M(l);r+=`
            <tr>
              <td>${h(a.key)}</td>
              <td class="betting-cell" data-event="${e.away_team} @ ${e.home_team}" data-event-time="${e.commence_time}" data-bet-type="${h(a.key)}" data-selection="${s.name}" data-odds="${u.toFixed(2)}" data-line="${s.point?s.point:"-"}">${s.name}</td>
              <td>${l.toFixed(2)}</td>
  ...
  `,t.events.forEach((e,n)=>{const i=document.getElementById(`event-card-${n}`),c=document.getElementById(`event-content-${n}`);i&&c&&(c.style.display="none",i.classList.add("collapsed"),i.addEventListener("click",function(a){a.target.closest(".event-content")||(c.style.display==="none"?(c.style.display="block",i.classList.remove("collapsed")):(c.style.display="none",i.classList.add("collapsed")))}))}),document.querySelectorAll(".betting-cell").forEach(e=>{e.addEventListener("click",function(n){n.stopPropagation(),C(this)})}),document.getElementById("closeBettingModal").addEventListener("click",B),document.getElementById("closeBettingExampleModal").addEventListener("click",$),document.getElementById("bettingExampleBtn").addEventListener("click",()=>{document.getElementById("bettingExampleModal").style.display="flex"});const d=document.getElementById("bettingProofImage");document.getElementById("bettingForm").addEventListener("paste",e=>{if(e.clipboardData&&e.clipboardData.files&&e.clipboardData.files.length>0){const n=e.clipboardData.files[0];if(n.type.startsWith("image/")){const i=new DataTransfer;i.items.add(n),d.files=i.files}}}),document.getElementById("bettingForm").addEventListener("submit",P)}function M(t){const o=t*.8;return Math.max(o,1.01)}function h(t){return{h2h:"Moneyline",spreads:"Spread",totals:"Total"}[t]||t}let m=null;function C(t){const o=t.dataset.event,r=t.dataset.eventTime,d=t.dataset.betType,e=t.dataset.selection,n=t.dataset.odds,i=t.dataset.line;m={event:o,eventTime:r,betType:d,selection:e,odds:n,line:i},document.getElementById("modalEventTitle").textContent=`${o} - ${d}`;const c=document.getElementById("teamSelection");if(c.innerHTML="",d==="Moneyline")o.split(" @ ").forEach(s=>{const l=document.createElement("button");l.type="button",l.textContent=s,l.style.cssText="padding:0.5rem 1rem; background:#333; color:#fff; border:1px solid #444; border-radius:5px; cursor:pointer; flex:1;",l.addEventListener("click",function(){c.querySelectorAll("button").forEach(u=>{u.style.background="#333",u.style.borderColor="#444"}),this.style.background="#646cff",this.style.borderColor="#646cff",m.selectedTeam=this.textContent}),c.appendChild(l)});else{const a=document.createElement("button");a.type="button",a.textContent=e,a.style.cssText="padding:0.5rem 1rem; background:#646cff; color:#fff; border:1px solid #646cff; border-radius:5px; cursor:pointer; width:100%;",m.selectedTeam=e,c.appendChild(a)}document.getElementById("bettingForm").reset(),document.getElementById("bettingError").textContent="",document.getElementById("bettingSuccess").textContent="",document.getElementById("bettingModal").style.display="flex"}function B(){document.getElementById("bettingModal").style.display="none",m=null}function $(){document.getElementById("bettingExampleModal").style.display="none"}async function P(t){if(t.preventDefault(),!m||!m.selectedTeam){document.getElementById("bettingError").textContent="Please select a team.";return}const o=document.getElementById("bettingUsername").value,r=document.getElementById("bettingAmount").value,d=document.getElementById("bettingProofImage").files[0];if(!d){document.getElementById("bettingError").textContent="Image required.";return}const e="321fcbefd94f6d6936d225a7c1004060",n=await new Promise((a,s)=>{const l=new FileReader;l.onload=()=>a(l.result.split(",")[1]),l.onerror=s,l.readAsDataURL(d)});let i="";try{const s=await(await fetch(`https://api.imgbb.com/1/upload?key=${e}`,{method:"POST",body:new URLSearchParams({image:n})})).json();if(s.success)i=s.data.url;else{document.getElementById("bettingError").textContent="Image upload failed.";return}}catch{document.getElementById("bettingError").textContent="Image upload failed.";return}const c={username:o,amountBet:r,eventName:m.event,eventTime:m.eventTime,betType:m.betType,selectedTeam:m.selectedTeam,odds:m.odds,line:m.line,proofImageUrl:i,adminUsername:p};try{const a=await fetch(`${y}/api/submit-sports-bet`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)});if(a.ok)document.getElementById("bettingSuccess").textContent="Bet placed successfully!",document.getElementById("bettingError").textContent="",document.getElementById("bettingForm").reset(),setTimeout(()=>{B()},2e3);else{const s=await a.json();document.getElementById("bettingError").textContent=s.message||"Failed to place bet.",document.getElementById("bettingSuccess").textContent=""}}catch{document.getElementById("bettingError").textContent="Failed to place bet.",document.getElementById("bettingSuccess").textContent=""}}window.toggleEventCard=function(t){console.log("Toggle called for index:",t);const o=document.getElementById(`event-content-${t}`),r=o.closest(".event-card");console.log("Content element:",o),console.log("Current display style:",o.style.display),o.style.display==="none"?(o.style.display="block",r.classList.remove("collapsed"),console.log("Expanding card")):(o.style.display="none",r.classList.add("collapsed"),console.log("Collapsing card"))};b?f():x();
