(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const r of e)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&a(d)}).observe(document,{childList:!0,subtree:!0});function n(e){const r={};return e.integrity&&(r.integrity=e.integrity),e.referrerPolicy&&(r.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?r.credentials="include":e.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(e){if(e.ep)return;e.ep=!0;const r=n(e);fetch(e.href,r)}})();let g=!1,p="";const b="https://admin-site-ze7d.onrender.com",y="df860f2526805007d06289b53d901e26",h="https://api.the-odds-api.com/v4";function v(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("loginForm").onsubmit=async t=>{t.preventDefault();const o=document.getElementById("loginUsername").value,n=document.getElementById("loginPassword").value,a=await fetch(`${b}/api/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:o,password:n})});if(a.ok)g=!0,p=o,f();else{const e=await a.json();document.getElementById("loginError").innerText=e.message||"Login failed"}}}function f(){document.querySelector("#app").innerHTML=`
  ...
  `,document.getElementById("exampleBtn").onclick=()=>{document.getElementById("exampleModal").style.display="flex"},document.getElementById("closeExampleModal").onclick=()=>{document.getElementById("exampleModal").style.display="none"},document.getElementById("backToMenuBtn").onclick=()=>{f()};const t=document.getElementById("proofImage");document.getElementById("playerForm").addEventListener("paste",o=>{if(o.clipboardData&&o.clipboardData.files&&o.clipboardData.files.length>0){const n=o.clipboardData.files[0];if(n.type.startsWith("image/")){const a=new DataTransfer;a.items.add(n),t.files=a.files}}}),document.getElementById("playerForm").onsubmit=async o=>{o.preventDefault();const n=document.getElementById("playerUsername").value,a=document.getElementById("amountToAdd").value,e=document.getElementById("proofImage").files[0];if(!e){document.getElementById("formError").innerText="Image required.";return}const r="321fcbefd94f6d6936d225a7c1004060",d=await new Promise((i,m)=>{const u=new FileReader;u.onload=()=>i(u.result.split(",")[1]),u.onerror=m,u.readAsDataURL(e)});let l="";try{const m=await(await fetch(`https://api.imgbb.com/1/upload?key=${r}`,{method:"POST",body:new URLSearchParams({image:d})})).json();if(m.success)l=m.data.url;else{document.getElementById("formError").innerText="Image upload failed.";return}}catch{document.getElementById("formError").innerText="Image upload failed.";return}const s={playerUsername:n,amountToAdd:a,proofImageUrl:l,adminUsername:p},c=await fetch(`${b}/api/submit`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(c.ok)document.getElementById("formSuccess").innerText="Submitted successfully!",document.getElementById("formError").innerText="",document.getElementById("playerForm").reset();else{const i=await c.json();document.getElementById("formError").innerText=i.message||"Submission failed.",document.getElementById("formSuccess").innerText=""}}}async function I(){try{const t=await fetch(`${h}/sports?apiKey=${y}`);if(!t.ok)throw new Error(`HTTP error! status: ${t.status}`);const o=await t.json(),n=document.getElementById("sportSelect");n.innerHTML='<option value="">Select a sport...</option>',o.filter(e=>e.active).sort((e,r)=>e.group===r.group?e.title.localeCompare(r.title):e.group.localeCompare(r.group)).forEach(e=>{const r=document.createElement("option");r.value=e.key,r.textContent=`${e.group} - ${e.title}`,n.appendChild(r)})}catch(t){console.error("Error loading sports:",t),document.getElementById("sportSelect").innerHTML='<option value="">Error loading sports</option>'}}async function x(t){const o=document.getElementById("oddsContainer");o.innerHTML="<p>Loading odds...</p>";try{const n=await fetch(`${h}/sports/${t}/odds?regions=us&oddsFormat=decimal&markets=h2h,spreads,totals&apiKey=${y}`);if(!n.ok)throw new Error(`HTTP error! status: ${n.status}`);const a=await n.json(),e={title:S(t),events:a};T(e)}catch(n){console.error("Error loading odds:",n),o.innerHTML="<p>Error loading odds. Please try again.</p>"}}function S(t){return{americanfootball_nfl:"NFL",americanfootball_ncaaf:"NCAAF",basketball_nba:"NBA",basketball_wnba:"WNBA",baseball_mlb:"MLB",icehockey_nhl:"NHL",soccer_epl:"EPL",soccer_usa_mls:"MLS",mma_mixed_martial_arts:"MMA"}[t]||t}function T(t){const o=document.getElementById("oddsContainer");if(!t.events||t.events.length===0){o.innerHTML="<p>No events available for this sport at the moment.</p>";return}let n=`<h3>${t.title} - Upcoming Games</h3>`;t.events.forEach((a,e)=>{const r=new Date(a.commence_time).toLocaleString();n+=`
    ...
    `,a.bookmakers.forEach(d=>{n+=`
        <div class="bookmaker-section">
          ...
          <table class="odds-table">
            ...
            <tbody>
      `,d.markets.forEach(l=>{l.outcomes.forEach(s=>{const c=s.price,i=w(c);n+=`
            <tr>
              <td>${L(l.key)}</td>
              <td>${s.name}</td>
              <td>${c.toFixed(2)}</td>
    ...
    `}),o.innerHTML=n}function w(t){const o=t*.8;return Math.max(o,1.01)}function L(t){return{h2h:"Moneyline",spreads:"Spread",totals:"Total"}[t]||t}g?f():v();
